require 'spec_helper'

describe Spree::PaymentMethod::PayflowPro do
  subject { described_class.create!(name: 'PayflowPro') }

  context '.gateway_class' do
    it 'is a Payflow gateway' do
      expect(subject.gateway_class).to eq ::ActiveMerchant::Billing::PayflowGateway
    end
  end

  describe 'options' do
    it 'include :test => true when :test_mode is true' do
      subject.preferred_test_mode = true
      expect(subject.options[:test]).to be true
    end

    it 'does not include :test when test_mode is false' do
      subject.preferred_test_mode = false
      expect(subject.options[:test]).to be false
    end
  end
end
